...
| Context Signals | README "API" | a published Context Signal is held and compared one level deep, so a structurally equal signal neither republishes nor wakes a Context Target; nested objects and every non-plain value compare by identity |
| Header Actions | README "API" → "Header Actions" | the two `Canvas.Action` shapes and their mutual exclusion at the type level; that both sort into one row by `priority` then `id`; that content is registered rather than portalled, and therefore re-registers nothing when it re-renders, resolves React context at the header, and runs in its own Panel's scope; that content may register nothing further; that `content` does not accept `undefined`; that content which throws is dropped from the row and reported through `onRendererError`; and that a button Action's optional `icon` and `description` belong to the button shape alone, that `label` remains the whole accessible name whatever icon is registered, that `description` is rendered whenever it is supplied rather than only while the Action is disabled, and that it is announced but not visible while remaining part of the button's DOM text |
| Activation and focus | README "Navigation", "Accessibility" | that focus does not imply Activation; the optional `activateOnFocus` on a Canvas Workspace and its `false` default; that it activates a retained Panel from a primary pointer press anywhere inside it or from focus arriving on its own; that Canvas-placed focus and interaction behind an open dialog do not activate; and that activation through either route never moves focus |
