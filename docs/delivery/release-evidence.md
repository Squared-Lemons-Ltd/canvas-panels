...
**The cause is established.** `changeset publish` writes an *annotated* tag — `git tag <name> -m <name>`, deliberately, so that `--follow-tags` would work — and an annotated tag requires a committer identity. The publish job has none: `actions/checkout` sets `http.extraheader`, `safe.directory` and `gc.auto` and no identity, nothing else in `release.yml` sets one, and git's hostname fallback on a GitHub-hosted runner derives `runner@<host>.(none)`, which git refuses. `git tag` exits 128 having written nothing.
