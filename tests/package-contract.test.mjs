...
test("only the release workflow can publish, and it holds no publish token", async () => {
  ...
  // The workflow publishes; it does not version. This organization forbids
  // GitHub Actions from creating pull requests, so a `version:` step could only
  // ever fail — and a release that fails every time is worse than one that does
  // not run, because it teaches everyone to ignore a red release. Versioning is
  // a local `pnpm release:version`, reviewed as the commit that carries it.
  assert.doesNotMatch(executed, /version:\s*pnpm release:version/);
  assert.doesNotMatch(executed, /pull-requests:\s*write/);
  assert.match(executed, /publish:\s*pnpm release:publish/);
});
