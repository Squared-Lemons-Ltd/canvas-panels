...
`description` supplies the announced half only: the package does not render a visible explanation. When sighted users also need the reason in the Panel, that layout remains the application's to render. Derive both the visible reason and the Action's `description` from one application-owned source so the two cannot drift.

The visually-hidden element is still a descendant of the button, so `button.textContent` includes the hidden description and a markup snapshot includes its element. To assert the control's name, prefer an accessible query for the button's `label`; to inspect the visible label specifically, read the button's own text nodes rather than all descendant text.
...
**Focus does not imply Activation.** Tabbing into a retained Panel, or pressing a focusable control inside it, focuses it and makes it the DOM-Focused Panel — but that does not make it the Active Panel. A primary pointer press on non-focusable Panel chrome moves neither focus nor Activation by default. A Panel becomes active when something navigates: opening from it, closing back to it, restoring a stack, or a host calling `activate` itself. Until then the DOM-Focused Panel keeps its retained width and styling, and every `useNavigation()`, `usePanel()`, or `usePresentation()` call that defaults to "the Active Panel" resolves somewhere else. A host that wants click-to-activate can use the option below or call `activate` itself.
...
`activateOnFocus` defaults to `false`, so a Canvas that does not ask keeps the rule above exactly. With it on, a primary pointer press anywhere inside a retained Panel makes it the Active Panel; so does focus arriving inside it on its own, by Tab or by F6. Focus the Canvas places itself — returning it to the control that opened a Guarded Transition or rescuing it out of a Panel the presentation has just hidden — activates nothing, because a repair is not an arrival. Neither a press nor focus reaching a Panel behind an open Guarded Transition dialog activates it, because the Panels are inert. Activating this way is silent, like every other activation, and **never moves focus**: a press on Panel chrome leaves focus where it was, a pressed control takes normal browser focus, and focus that arrived by Tab or F6 stays where the user put it. See "Accessibility".
